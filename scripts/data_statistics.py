...
def type_language_statistics(dir_path, result_path):
    ...
    try:
        ...
        supplementary_dataset_dir = 'supplementary-dataset'
...
def abstract_stats(dataset_path, result_path):
    """
    Extract the statistics (min, max and mean) for all abstracts present in the final SemEval dataset
    ...
    try:
        ...
        #List to store abstract statistics in text format so to write on a text file
        text = []
        ...
        for root, dir, files in os.walk(dataset_path):
            if files:
                ...
                for file in files:
                    ...
                    if type_lang_key not in abstract_stats.keys():
                        abstract_stats[type_lang_key] = []
                    abstract_stats[type_lang_key].append(tokens)
        
        for key, value in abstract_stats.items():
            text.append('{} - Min token: {}, Avg tokens: {:.1f}, Max tokens: {}\n'.format(key, min(value), mean(value), max(value)))
        file_saved = write_text_file('{}/abstract_stats.txt'.format(result_path), text)
...
def subject_statistics(dataset_path, result_path):
    """
    Extract the statistics for dc:subjects present in the final SemEval dataset
    ...
    try:
        ...
        #List to store total subjects in text format so to write on a text file
        total_subject_text = []
        ...
        for root, dir, files in os.walk(dataset_path):
            if files:
                ...
                for file in files:
                    ...
                    if type_lang_key not in dcsubject_stats.keys():
                        dcsubject_stats[type_lang_key] = []
                    dcsubject_stats[type_lang_key].append(total_subjects)
        
        for key, value in dcsubject_stats.items():
            total_subject_text.append('{} - Min: {}, Avg: {:.1f}, Max: {}\n'.format(key, min(value), mean(value), max(value)))
        file_saved = write_text_file('{}/subject_stats.txt'.format(result_path), total_subject_text)
...
if __name__ == '__main__':
    ...
    gnd_subjects = read_json_file('../gnd-how-to/gnd-subjects/GND-Subjects-all.json')
    ...
    abstract_stats(final_dataset_path, '../analysis')

    #Creating dc:subjects statistics file
    print('\nCreating Subject\'s statistics file')
    subject_statistics(final_dataset_path, '../analysis')

    #Creating dc:subject frequencies file
    print('\nCreating Subject Frequencies file')
    subject_frequencies(final_dataset_path, '../analysis', gnd_subjects)
    ...
    unique_subjects(final_dataset_path, '../analysis/unique-subjects', gnd_subjects)